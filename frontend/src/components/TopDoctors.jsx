    ...
    return (
        <section className='mf-section'>
            <div className='flex flex-col items-center gap-2 text-center'>
              <p className='mf-eyebrow'>Clinicians you can trust</p>
              <h2 className='mf-title'>Meet your care team</h2>
              <p className='mf-copy'>Availability, speciality, and booking stay clear from the first look.</p>
            </div>
            ...
            {!doctorsLoading && !doctorsError && doctors.length === 0 && <div className='mt-8 w-full border-y border-line bg-white px-6 py-10 text-center'>
                <p className='text-lg font-semibold text-ink'>New clinician profiles are being prepared</p>
                <p className='mt-2 text-sm text-slate-600'>There are no available clinicians to display yet. Please check again soon.</p>
            </div>}
            {doctors.length > 0 && <button onClick={() => { navigate('/doctors'); scrollTo(0, 0) }} className='mf-button-secondary mt-8'>Browse all doctors</button>}
        </section>
