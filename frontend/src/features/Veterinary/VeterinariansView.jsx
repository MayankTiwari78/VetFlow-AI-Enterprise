...
const nextAvailabilityLabel = (doctor) => {
  ...
  const enabled =
    ...
    (doctor.availability?.enabled === undefined || doctor.availability.enabled !== false);
...
const VeterinariansView = () => {
  const navigate = useNavigate();
  const { authStatus, token, doctors, doctorsLoading, doctorsError, getDoctosData, currencySymbol } =
  ...
  const applyFilter = useMemo(() => {
    ...
    return doctors.filter((doc) => {
      if (selectedSpeciality && normalizeSpeciality(doc.speciality) !== normalizeSpeciality(selectedSpeciality)) {
        return false;
      }
  ...
  }, [doctors, selectedSpeciality, searchTerm, availableOnly]);
  ...
  const hasActiveFilters = Boolean(searchTerm.trim()) || availableOnly || Boolean(selectedSpeciality);
    ...
    <section className="w-full min-w-0">
      ...
      <div className="mb-8">
        ...
        <p className="mt-3 max-w-2xl text-base leading-7 text-muted">
          Filter by clinical department and choose a veterinarian for your pet. Selecting a doctor
          takes you to the booking flow.
        </p>
      </div>
        ...
        >
          ...
          <button
            ...
            className={`rounded-xl px-3.5 py-2.5 text-left text-sm font-semibold transition ${
              !selectedSpeciality ? "bg-[#E7F4F5] text-primary" : "text-muted hover:bg-mist hover:text-ink"
            }`}
          >
            All Departments
          </button>
          {vetSpecialities.map((item) => (
            <button
              ...
              type="button"
              onClick={() =>
                setSelectedSpeciality(normalizeSpeciality(selectedSpeciality) === normalizeSpeciality(item) ? "" : item)
              }
              className={`rounded-xl px-3.5 py-2.5 text-left text-sm font-semibold transition ${
                normalizeSpeciality(selectedSpeciality) === normalizeSpeciality(item)
              ...
              return (
                ...
                >
                  ...
                  <div className="relative h-[200px] w-full shrink-0 overflow-hidden bg-[#E7F4F5]">
                    ...
                    <span
                      className={`absolute left-3.5 top-3.5 z-10 inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold shadow-soft backdrop-blur-md ${
                        item.available ? "bg-emerald-500/95 text-white" : "bg-slate-700/90 text-white"
                      }`}
                    >
                      <span className={`h-1.5 w-1.5 rounded-full ${item.available ? "bg-white" : "bg-slate-300"}`} />
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    ...
                    {/* Book — preserves doctor ID into existing booking flow */}
                    <button
                      type="button"
                      onClick={() => {
...
export default VeterinariansView;